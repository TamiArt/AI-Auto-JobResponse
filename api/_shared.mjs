...
async function loadPublicSnapshot(source) {
  ...
  if (source === "remoteok") {
    ...
    const payload = await fetchWithTimeout(API_URLS.arbeitnow);
    jobs = Array.isArray(payload?.data) ? payload.data.map((job) => ({
      ...
      publishedTimestamp: typeof job?.created_at === "number" ? job.created_at * 1000 : Date.parse(String(job?.created_at || "")) || 0,
