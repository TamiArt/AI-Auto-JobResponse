...
export function CareerPanel({ profile, onChange }: Props) {
  ...
  return (
    <div className="max-w-3xl space-y-4">
      <div><h1 className="text-lg font-bold">Career Graph</h1><p className="text-xs font-mono text-muted-foreground mt-1">Один редактируемый источник правды о вашей карьере.</p></div>
      <section className="rounded-xl border border-border bg-card p-5 space-y-4">
        <div className="grid md:grid-cols-2 gap-4">
          <label className="space-y-1.5"><span className="text-[10px] font-mono uppercase text-muted-foreground">Имя</span><input value={profile.name} onChange={(e) => update({ name: e.target.value })} className="w-full rounded-xl border border-border bg-input-background px-3 py-2.5 text-sm" /></label>
