...
export function AiPanel({ profile }: Props) {
  ...
  return (
    <div className="max-w-4xl space-y-4">
      <div><h1 className="text-lg font-bold">AI Career Studio</h1><p className="text-xs font-mono text-muted-foreground mt-1">Career Graph + вакансия превращаются в безопасный, проверяемый AI-контекст.</p></div>
      <section className="rounded-xl border border-border bg-card p-5 space-y-4">
        ...
        <div className="grid md:grid-cols-2 gap-3">
          <div className="rounded-xl border border-border p-4"><div className="text-sm font-semibold">Открыть в Gemini</div><p className="text-xs text-muted-foreground mt-1">Запрос копируется в буфер, Gemini открывается отдельно. API-ключ не нужен.</p><button onClick={openChat} className="mt-3 rounded-xl bg-[var(--neon-violet)] px-4 py-2 text-sm text-white">Скопировать и открыть Gemini</button></div>
          <div className="rounded-xl border border-border p-4"><div className="text-sm font-semibold">Gemini API</div><p className="text-xs text-muted-foreground mt-1">Ключ используется только для текущего запроса и не сохраняется приложением.</p><input type="password" autoComplete="off" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="Gemini API key" className="mt-3 w-full rounded-xl border border-border bg-input-background px-3 py-2 text-sm" /><button disabled={!apiKey || busy} onClick={runGemini} className="mt-2 rounded-xl border border-border px-4 py-2 text-sm disabled:opacity-50">{busy ? "Анализ…" : "Запустить Gemini"}</button></div>
        </div>
        {error && <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-3 text-sm">{error}</div>}
        {result && <div className="rounded-xl border border-border bg-background p-4 whitespace-pre-wrap text-sm leading-6">{result}</div>}
      </section>
    </div>
  );
}
