...
import type { Config, ExperienceFilter, Theme } from "./domain/types";
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col" style={{ fontFamily: "Inter, sans-serif" }}>
      ...
      <header className="relative z-10 border-b border-border bg-card/80 backdrop-blur-sm sticky top-0">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
          <button type="button" onClick={() => setTab("search")} className="flex items-center gap-2.5"><AppIcon /><span className="font-bold tracking-tight" style={{ fontFamily: "Oxanium, monospace", fontSize: "1.1rem" }}>HuntPulse<span className="text-[var(--neon-violet)]">_</span>AI</span></button>
        ...
        <div className="md:hidden flex border-t border-border">{navItems.map((item) => <button key={item.id} onClick={() => setTab(item.id)} className={`flex-1 flex flex-col items-center gap-0.5 py-2 px-2 text-[10px] font-mono ${tab === item.id ? "text-[var(--neon-violet)]" : "text-muted-foreground"}`}>{item.icon}{item.label}</button>)}</div>
      </header>
          ...
          {tab === "settings" && (
            <motion.div key="settings" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
              <div className="max-w-2xl space-y-4">
                <div><h1 className="text-lg font-bold" style={{ fontFamily: "Oxanium, monospace" }}>Настройки поиска</h1><p className="text-xs font-mono text-muted-foreground mt-1">Здесь хранятся только локальные параметры поиска. API-ключи приложению не нужны.</p></div>
                <section className="rounded-xl border border-border bg-card p-5 space-y-4">
                  <Field label="Профессия по умолчанию" value={config.jobTitle} onChange={(value) => updateConfig({ jobTitle: value })} placeholder="QA-инженер, дизайнер, разработчик…" icon={<Target size={12} />} />
                  ...
                  <label className="block space-y-1.5"><span className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">Опыт по умолчанию</span><select value={config.experience} onChange={(event) => updateConfig({ experience: event.target.value as ExperienceFilter })} className="w-full px-3 py-2.5 rounded-xl border border-border bg-input-background text-foreground text-sm font-mono">{EXPERIENCE_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
      ...
      <footer className="relative z-10 border-t border-border py-3 px-4"><div className="max-w-6xl mx-auto flex items-center justify-between text-[10px] font-mono text-muted-foreground"><span>HuntPulse AI · поиск вакансий</span><span>без платных API</span></div></footer>
    </div>
  );
}
