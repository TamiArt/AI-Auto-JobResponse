...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col" style={{ fontFamily: "Inter, sans-serif" }}>
          ...
          {tab === "settings" && (
            <motion.div key="settings" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
              <div className="max-w-2xl space-y-4">
                <div><h1 className="text-lg font-bold" style={{ fontFamily: "Oxanium, monospace" }}>Настройки поиска</h1><p className="text-xs font-mono text-muted-foreground mt-1">Параметры поиска и публичные источники. Секреты не сохраняются в настройках.</p></div>
      ...
      <footer className="relative z-10 border-t border-border py-3 px-4"><div className="max-w-6xl mx-auto flex items-center justify-between text-[10px] font-mono text-muted-foreground"><span>JOBOS AI · Career Operating System</span><span>Web + Telegram Mini App ready</span></div></footer>
    </div>
  );
}
